...
_STRUCTURE_PDB_PATH = flags.DEFINE_string(
    'structure_pdb_path',
    None,
...
_JAX_COMPILATION_CACHE_DIR = flags.DEFINE_string(
    ...
    '/home/ge/app/alphafold3/jax_compile',
...
class ModelRunner:
  ...
  def _model(
    ...
    def forward_fn(batch):
      # print(batch['pseudo_beta_positions']) #[241,3]
      return model.Model(self._model_config, only_confidence = True)(batch)
  ...
  #     self,
  # ) -> Callable[[jnp.ndarray, features.BatchDict], model.ModelResult]:
  #     """Loads model parameters and returns a model forward pass without jitting."""
  ...
  #     return functools.partial(forward_fn.apply, self.model_params)

  def get_pseudo_betas_from_pdb(self,
            pdb_file: str):
        ...
        if cb_coords:
            cb_array = np.stack(cb_coords)  # (N, 3)
        else:
            cb_array = np.zeros((0, 3))

        if ligand_coords:
            ligand_array = np.stack(ligand_coords)  # (M, 3)
        else:
            ligand_array = np.zeros((0, 3))

        combined = np.concatenate([cb_array, ligand_array], axis=0)

        return combined

  def run_inference(
      self, featurised_example: features.BatchDict, rng_key: jnp.ndarray, structure_pdb_path: os.PathLike[str] | None = None
  ) -> model.ModelResult:
    """Computes a forward pass of the model on a featurised example."""
    if structure_pdb_path is not None:
      pseudo_beta_positions = self.get_pseudo_betas_from_pdb(structure_pdb_path)

    featurised_example['pseudo_beta_positions'] = pseudo_beta_positions
...
def predict_structure(
    ...
    structure_pdb_path: os.PathLike[str] | None = None,
  ...
  for seed, example in zip(fold_input.rng_seeds, featurised_examples):
    ...
    result = model_runner.run_inference(example, rng_key, structure_pdb_path)
    print(
    ...
    if not only_confidence:
      ...
      pred_structure = get_predicted_structure(result=result, batch=batch, only_confidence=only_confidence)
...
def process_fold_input(
  ...
  if model_runner is None:
    ...
    with open(f'{output_dir}/{fold_input.sanitised_name()}_atomplddt.json', 'w') as f:
        json.dump({'atom_plddts':all_inference_results.tolist()}, f, indent=4)
    # write_outputs(
...
def main(_):
  ...
  if _STRUCTURE_PDB_PATH.value is not None:
    stru_pdb_path = _STRUCTURE_PDB_PATH.value
  else:
    stru_pdb_path = None
  ...
  for fold_input in fold_inputs:
    ...
    process_fold_input(
        ...
        # Optional arguments.
        structure_pdb_path=stru_pdb_path,
    )
    num_fold_inputs += 1
