...
def visualize_2d_logits(logits, title="Logits Heatmap", out_path=''):
    """
    logits: [L, C] tensor
    """
    ...
    im = plt.imshow(logits_np, aspect='auto', cmap='viridis')
    L,C = logits_np.shape
...
def main(args) -> None:
    ...
    # loop over PDB paths
    feature_dict_list = []
    multistate_design = args.multistate_design
    if multistate_design == True:
        for pdb in pdb_paths:
            if args.verbose:
        ...
        with torch.no_grad():
            ...
            for _ in range(args.number_of_batches):
                for i in range(len(feature_dict_list)):
                    feature_dict_list[i]["randn"] = torch.randn(
                        [feature_dict_list[i]["batch_size"], feature_dict_list[i]["mask"].shape[1]],
                        device=device,
                    )
                # output_dict = model.sample(feature_dict)
                output_dict = model.multisate_sample(feature_dict_list)
            ...
            out_png = base_folder + "pngs/" + name + args.file_ending + ".png"
            if not os.path.exists(base_folder + "pngs/"):
                os.makedirs(base_folder + "pngs/", exist_ok=True)
            visualize_2d_logits(output_dict['sampling_probs'][0],out_path=out_png )
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        ...
        type=str,
        default="/home/ge/app/MSD_design/LigandMPNN_MSD/model_params/proteinmpnn_v_48_020.pt",
        help="Path to model weights.",
    )
    argparser.add_argument(
        "--checkpoint_ligand_mpnn",
        type=str,
        default="/home/ge/app/MSD_design/LigandMPNN_MSD/model_params/ligandmpnn_v_32_010_25.pt",
        help="Path to model weights.",
    )
    argparser.add_argument(
        "--checkpoint_per_residue_label_membrane_mpnn",
        type=str,
        default="/home/ge/app/MSD_design/LigandMPNN_MSD/model_params/per_residue_label_membrane_mpnn_v_48_020.pt",
        help="Path to model weights.",
    )
    argparser.add_argument(
        "--checkpoint_global_label_membrane_mpnn",
        type=str,
        default="/home/ge/app/MSD_design/LigandMPNN_MSD/model_params/global_label_membrane_mpnn_v_48_020.pt",
        help="Path to model weights.",
    )
    argparser.add_argument(
        "--checkpoint_soluble_mpnn",
        type=str,
        default="/home/ge/app/MSD_design/LigandMPNN_MSD/model_params/solublempnn_v_48_020.pt",
        help="Path to model weights.",
    )
    ...
    argparser.add_argument(
        "--checkpoint_path_sc",
        type=str,
        default="/home/ge/app/MSD_design/LigandMPNN_MSD/model_params/ligandmpnn_sc_v_32_002_16.pt",
        help="Path to model weights.",
    )
    ...
    argparser.add_argument(
        ...
        type=bool,
        default=False,
        help="whether to use proteinMPNN-MSD.",
    )
